...
$parts = ($decodedData['parts']);


$SQL = "SELECT name FROM seller WHERE name = '$name'";
...
if ($check == 0&&$name) {
    ...
    if($check1==0)
    {
        ...
        $locid=$row['id'];
        $SQL = "SELECT id FROM parts WHERE type='$parts' ";
        $exeSQL = mysqli_query($con, $SQL);
        $row = mysqli_fetch_array($exeSQL);
        $pid=$row['id'];
        $SQL = "INSERT INTO seller (name,locid,parts,password) VALUES('$name','$locid','$pid','$pass')";
